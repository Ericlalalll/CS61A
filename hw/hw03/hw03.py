...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def pingpong_helper(ans, i, counter): # count the pinponge value of ith number
        if i == n:
            return ans
        return pingpong_helper(ans + counter, i + 1, dir_checker(counter, i+1))
    def dir_checker(counter, i):
...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_helper(change, largest_coin):
        if change < 0:
            return 0
        if change < 5:
            return 1
        if largest_coin == None:
            return 0
        without_coin = count_helper(change, next_smaller_coin(largest_coin))
        with_coin = count_helper(change - largest_coin, largest_coin)
