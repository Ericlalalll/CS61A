...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        expressions_combined = Pair(formals, body)
        env.define(name, do_lambda_form(expressions_combined, env))
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if is_scheme_true(first_expr):  
            return first_expr
        elif is_scheme_false(first_expr):  
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    env.make_child_frame(names, vals)
    # END PROBLEM 14
    while bindings is not nil:
        pair = bindings.first
        validate_form(pair, 2, 2)
        names = Pair(pair.first, names)
        vals = Pair(eval_all(pair.rest, env), vals)
        bindings = bindings.rest
    validate_formals(names)
    return env.make_child_frame(names, vals)
    # from the spec of let, can see we should firstly eval each sub-exp, then do the bind temp
 #The bindings Scheme list contains pairs that each contain a symbol and a corresponding expression.
