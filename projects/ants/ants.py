...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if place.bees and self.lower_bound <= distance <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount == self.health:
            amount_plus = amount + self.damage
        if self.place.bees:
            for bee in self.place.bees[:]:
                bee.reduce_health(amount_plus)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            ...
            bee.health = 0
            bee.reduce_health(0) # this func will cause the bee being removed
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not gamestate.queen_occurs:
            gamestate.queen_occurs = True
