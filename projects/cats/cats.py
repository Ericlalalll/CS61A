...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    vaild_para = [para for para in paragraphs if select(para)]
    if k < len(vaild_para):
        return vaild_para[k]
    else:
        return ''
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def if_contains(paragraph):
        for i in split(lower(remove_punctuation(paragraph))):
            if i in topic:
                return True
        return False
    return if_contains
...
def feline_fixes(typed, source, limit):
    ...
    def count_helper(start, goal, count):
        if count > limit:
            return limit + 1
        if len(start) > len(goal):
            return count_helper(goal, start, count)  # assuming goal is longer
        if len(start) == 0:  
            return count + len(goal)
        elif start[0] == goal[0]:
            return count_helper(start[1:], goal[1:], count)    # matched for current char
        else:
            return count_helper(start[1:], goal[1:], count + 1)  # not matched for current char
    return count_helper(typed, source, 0)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0: 
        ...
        return len(start) + len(goal)  # as one of them is 0
    elif start[0] == goal[0]:
        return minimum_mewtations(start[1:], goal[1:], limit)
    else:
        add = 1 + minimum_mewtations((goal[0] + start)[1:], goal[1:], limit - 1)
...
def time_per_word(words, times_per_player):
    ...
    "*** YOUR CODE HERE ***"
    type_speed = []
    for time_block in times_per_player:
        timestamp_per_player = []
        for j in range(len(words)):
            timestamp_per_player += [time_block[j+1] - time_block[j]] 
        type_speed.append(timestamp_per_player)
    return match(words, type_speed)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        fastest_player, best_time = 0, time(match, 0, word_index)
        for player_index in player_indices:
            if time(match, player_index, word_index) < best_time:
                fastest_player, best_time = player_index, time(match, player_index, word_index)
        fastest_words_list[fastest_player].append(get_word(match, word_index))
